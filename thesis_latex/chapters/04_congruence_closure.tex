...
\begin{exmp}
Let the proof forest be the following:

2 $\xrightarrow{2 = 3}$ 3 \ \ \ 1 $\xrightarrow{\text{1 = 0}}$ 0 \

TODO

2 $\xrightarrow{2 = 3}$ 3 $\xrightarrow[\text{F(2,0) = 1}]{\text{F(2,0) = 3}}$ 1 $\xrightarrow{\text{1 = 0}}$ 0 \
\end{exmp}
...
The same invariant holds for the labels in \lstinline|pf_labels|. This is easy to prove,  because thy are only modified in \lstinline|propagate_step|, where the added label comes from \lstinline|pending|, therefore the invariant follows from \lstinline|pending_invar|.
...
Similarly to the \lstinline{explain} operation for union-find (see Section \ref{subsection:uf-correctness}), we can prove that the equations in the output of \lstinline{cc_explain} are a subset of the input equations.
...
From this lemma we can easily show the theorem \lstinline{cc_explain_valid} by induction on \lstinline{cc_explain_aux}.
